...
**Row layout resize handles**: The `kadence.rowlayout.defaultPadding` hook provides the default value shown on padding resize handles. The module implementation returns 0 for rows without backgrounds (no default padding) and measures `--kb-row-default-padding` for rows with backgrounds. The measurement is cached per viewport width — one DOM measurement serves all background rows at a given width, avoiding scroll jitter from repeated synchronous reflows. Additionally, drag-to-resize is disabled via CSS (`pointer-events: none` on `.kt-padding-resize-box`) to prevent editors from setting arbitrary pixel values - forcing use of the design system's spacing scale.
